LABORATORIO 1 - PUNTO 1: ANÁLISIS FUNDAMENTAL DE LA TRM
PROYECTO: Derivados Financieros
FECHA: 26 de marzo de 2026
================================================================================

INTEGRANTES:
• Santiago Toro Cadavid - 1040739414
• Yenny Carolina Serna Chaverra - 1017210528
• Daniela Perez Meza - 1017220748

================================================================================
1. INFORMACIÓN GENERAL
================================================================================

1.1 Contexto del Análisis
-------------------------
Este estudio presenta un análisis completo de la Tasa Representativa del 
Mercado (TRM) Colombia-EE.UU. con un horizonte de proyección de 12 meses, 
partiendo de la tasa actual registrada el 26 de marzo de 2026.

[TABLA 1: Datos Generales del Análisis]

<img width="546" height="137" alt="image" src="https://github.com/user-attachments/assets/48cc3989-f604-40af-a730-41be9ae7667a" />


<img width="1094" height="293" alt="image" src="https://github.com/user-attachments/assets/b919f708-58cf-4b20-8968-7f2c04883df1" />

• Propósito: Mostrar la tendencia histórica con media móvil de 90 días
• Justificación: Permite visualizar el comportamiento cíclico y tendencia reciente

1.2 Análisis de la Situación Actual
-----------------------------------
La TRM actual de $3,688.46 pesos por dólar representa un nivel históricamente 
alto comparado con años anteriores. Observando el comportamiento histórico, 
la tasa de cambio ha mostrado una tendencia alcista pronunciada desde 2024, 
pasando de niveles cercanos a $2,000 COP/USD a superar los $3,600 en 2026.

Esta depreciación del peso colombiano indica que se necesitan más pesos para 
comprar un dólar, lo cual tiene implicaciones importantes para:
• Inflación interna
• Costo de importaciones
• Servicio de deuda externa

<img width="1092" height="335" alt="image" src="https://github.com/user-attachments/assets/6a0208ff-96ee-4101-bec6-1264dfd9d4fe" />

• Propósito: Mostrar la volatilidad diaria del mercado cambiario
• Justificación: Evidencia la incertidumbre y movimientos bruscos del mercado

================================================================================
2. FACTORES FUNDAMENTALES
================================================================================

2.1 Variables Macroeconómicas Clave
-----------------------------------
El comportamiento de la TRM está influenciado por múltiples factores económicos 
tanto nacionales como internacionales.

[TABLA 2: Factores Fundamentales del Análisis]

<img width="815" height="187" alt="image" src="https://github.com/user-attachments/assets/e6592298-852f-48d0-8c7c-fe4bdb5716dd" />

<img width="3570" height="1783" alt="punto1_analisis_tecnico_trm" src="https://github.com/user-attachments/assets/64cc5997-cb4d-4d66-bfd2-a9aecb372221" />

• Mostrar: Tasas de interés BanRep vs Fed, inflación USA, precio petróleo
• Propósito: Visualizar comparación de indicadores clave

2.2 Análisis Detallado de Factores
----------------------------------

a) Diferencial de Tasas de Interés (BanRep vs Fed)
• Análisis: Colombia mantiene tasa del 9.25%, superior a la Fed (5.25%). 
  Este diferencial de 4 puntos porcentuales debería atraer capitales 
  extranjeros, fortaleciendo el peso. Sin embargo, la TRM sigue alta, 
  lo que indica que otros factores de riesgo están desincentivando la 
  entrada de capitales.
• Referencia: Banco de la República (2026); Federal Reserve (2026)

b) Precio del Petróleo Brent ($60-65 USD/barril)
• Análisis: Colombia es exportador de petróleo. Precios moderado-bajos 
  significan menos ingresos en dólares, menor oferta de divisas y presión 
  al alza sobre la TRM.
• Referencia: U.S. Energy Information Administration (2026)

c) Riesgo Fiscal - Déficit del 7% del PIB
• Análisis: Un déficit del 7% es elevado según estándares internacionales. 
  Implica mayor necesidad de financiamiento, percepción de riesgo por 
  inversionistas y posible presión sobre la tasa de cambio.
• Referencia: Ministerio de Hacienda de Colombia (2026)

d) Contexto Electoral 2026 (Marzo-Junio)
• Análisis: El periodo electoral genera incertidumbre. Los inversionistas 
  adoptan postura de "esperar y ver", contribuyendo a mayor volatilidad 
  cambiaria y posible fuga de capitales.
• Referencia: Registraduría Nacional (2026)

e) Inflación en Estados Unidos (2.7%)
• Análisis: Inflación ligeramente por encima de la meta del 2% de la Fed, 
  pero manejable. Permite mantener tasas restrictivas sin subidas agresivas, 
  lo cual es positivo para Colombia.
• Referencia: U.S. Bureau of Labor Statistics (2026)

================================================================================
3. PROYECCIONES INSTITUCIONALES
================================================================================

3.1 Consenso del Mercado
------------------------
Se recopilaron proyecciones de 10 instituciones financieras y calificadoras 
de riesgo internacionales.

[TABLA 3: Proyecciones Institucionales de la TRM (12 meses)]

<img width="580" height="260" alt="image" src="https://github.com/user-attachments/assets/37cf4997-0c30-43a7-9292-4adb5869b409" />

Estadísticos Resumen:
• Mínimo proyectado: $3,650 COP/USD
• Máximo proyectado: $4,200 COP/USD
• Promedio (Central): $3,910 COP/USD
• Volatilidad implícita: 14.07%

<img width="4170" height="1783" alt="punto1_expectativa_dolar_12meses" src="https://github.com/user-attachments/assets/8ab2a889-947b-447d-a913-dd3f1bd10d4e" />

3.2 Análisis del Consenso
-------------------------
• El promedio de proyecciones centrales ($3,910) representa depreciación 
  esperada del 6.01% frente a la TRM actual.
• Amplio rango ($550 pesos) indica alta incertidumbre.
• 7 de 10 instituciones proyectan tasa central > $3,850 (sesgo alcista).
• Instituciones más conservadoras: BBVA Research, Banco de la República.
• Instituciones más agresivas: Citigroup, Fitch Ratings.

Referencia: Datos compilados de reportes públicos de cada institución (2026).

================================================================================
4. ESCENARIOS DE PROYECCIÓN
================================================================================

4.1 Distribución de Probabilidades
----------------------------------
Basado en el análisis de proyecciones institucionales y factores de riesgo.

[TABLA 4: Escenarios de Proyección TRM]

<img width="701" height="148" alt="image" src="https://github.com/user-attachments/assets/7490ed49-ef4f-49f2-948f-5e40c8f9a714" />

Expectativa Ponderada: $4,004 COP/USD
Variación Esperada: +8.55%

4.2 Descripción Detallada de Escenarios
---------------------------------------

▶ Escenario Base (55%) - TRM: $3,900
• Descripción: Continuación moderada de tendencia alcista actual.
• Supuestos: Diferencial de tasas mantenido, petróleo estable ($60-65), 
  incertidumbre electoral controlada, sin shocks externos.
• Implicaciones: Depreciación 5.7%, presión inflacionaria moderada.

▶ Escenario Alcista (25%) - TRM: $4,325
• Descripción: Mayor depreciación del peso.
• Supuestos: Petróleo < $55/barril, deterioro calificación crediticia, 
  incertidumbre post-electoral, dólar fuerte globalmente.
• Implicaciones: Depreciación 17.2%, presiones inflacionarias significativas, 
  posible intervención del BanRep.

▶ Escenario Bajista (15%) - TRM: $3,550
• Descripción: Fortalecimiento del peso colombiano.
• Supuestos: Petróleo > $75/barril, mejora percepción de riesgo, entrada 
  de capitales, resultados electorales favorables.
• Implicaciones: Apreciación 3.7%, alivio inflacionario, menor competitividad 
  exportadora.

▶ Escenario Crisis (5%) - TRM: $4,900
• Descripción: Estrés extremo.
• Supuestos: Crisis política/social, shock externo global, petróleo < $40, 
  fuga masiva de capitales.
• Implicaciones: Depreciación 32.9%, crisis inflacionaria, posible apoyo del FMI.

Referencia: Metodología basada en enfoques de análisis de escenarios del FMI.

================================================================================
5. ANÁLISIS TÉCNICO
================================================================================

5.1 Indicadores de Tendencia
----------------------------
Análisis técnico complementario mediante medias móviles exponenciales (EMA).

[TABLA 5: Indicadores Técnicos Actuales (26 de marzo de 2026)]

<img width="705" height="139" alt="image" src="https://github.com/user-attachments/assets/f5b7d70f-08f0-495b-916f-13ec5aa43ba4" />

5.2 Interpretación de los Indicadores
-------------------------------------
1. Posición relativa: TRM actual ($3,688.46) ligeramente por debajo de 
   EMA 20 ($3,689.87) → debilitamiento del momentum alcista corto plazo.

2. Cruce de medias: EMA 20 > EMA 50 indica tendencia alcista mediano plazo, 
   pero proximidad entre medias sugiere debilitamiento.

3. Señal bajista: Combinación de TRM < EMA 20 + reducción distancia entre 
   EMAs genera señal técnica bajista → posible consolidación próximas semanas.

4. Niveles clave:
   • Soporte inmediato: EMA 50 en $3,667
   • Resistencia: Máximos recientes ~$3,800-3,850

Referencia: Murphy, J.J. (1999). Technical Analysis of the Financial Markets.

================================================================================
6. CONCLUSIONES DEL PUNTO 1
================================================================================

6.1 Hallazgos Principales
-------------------------
✓ Tendencia de depreciación moderada: Todos los análisis apuntan a 
  depreciación moderada del peso en próximos 12 meses, con expectativa 
  ponderada de $4,004 COP/USD (+8.55%).

✓ Alta volatilidad esperada: Volatilidad implícita del 14.07% y amplio 
  rango de proyecciones reflejan entorno de alta incertidumbre.

✓ Factores de riesgo dominantes:
  - Riesgo fiscal elevado (déficit 7% PIB)
  - Incertidumbre periodo electoral 2026
  - Precios del petróleo moderado-bajos

✓ Señal técnica mixta: Fundamental sugiere depreciación, pero técnico 
  muestra señal bajista corto plazo → posible consolidación antes de 
  continuar tendencia alcista.

6.2 Recomendaciones para Monitoreo
----------------------------------
✅ Seguimiento mensual de:
   • Decisiones de tasas de BanRep y Fed
   • Precios del petróleo Brent
   • Indicadores de riesgo país (EMBI)
   • Encuestas de expectativas del mercado

✅ Alertas tempranas:
   • Si TRM > $3,850: activar escenario de cobertura
   • Si TRM < $3,550: reevaluar fundamentos
   • Si diferencial BanRep-Fed < 3%: revisar proyecciones

✅ Variables críticas a vigilar:
   • Resultados elecciones 2026 (marzo-junio)
   • Calificaciones de riesgo (Fitch, Moody's)
   • Política fiscal del gobierno

================================================================================
REFERENCIAS BIBLIOGRÁFICAS
================================================================================

1. Banco de la República de Colombia. (2026). Tasa Representativa del 
   Mercado (TRM). www.banrep.gov.co
2. Banco de la República de Colombia. (2026). Tasa de Intervención - 
   Subastas de Títulos.
3. Federal Reserve. (2026). Federal Funds Rate and Monetary Policy. 
   www.federalreserve.gov
4. U.S. Bureau of Labor Statistics. (2026). Consumer Price Index and 
   Inflation Data. www.bls.gov/cpi
5. U.S. Energy Information Administration. (2026). Brent Crude Oil Prices. 
   www.eia.gov
6. Ministerio de Hacienda de Colombia. (2026). Marco Fiscal de Mediano Plazo.
7. Fondo Monetario Internacional (FMI). (2026). World Economic Outlook y 
   Proyecciones para Colombia. www.imf.org
8. Banco Mundial. (2026). Global Economic Prospects. www.worldbank.org
9. Fitch Ratings, Moody's, S&P Global. (2026). Calificaciones Soberanas 
   de Colombia.
10. Goldman Sachs, JPMorgan Chase, Citigroup, BBVA Research. (2026). 
    Reportes de Investigación de Mercados Emergentes.
11. Registraduría Nacional del Estado Civil de Colombia. (2026). 
    Calendario Electoral 2026.
12. Murphy, J. J. (1999). Technical Analysis of the Financial Markets: 
    A Comprehensive Guide to Trading Methods and Applications. 
    New York Institute of Finance.
13. Mishkin, F. S. (2007). The Economics of Money, Banking and Financial 
    Markets. Pearson Education.
14. Krugman, P. R., Obstfeld, M., & Melitz, M. J. (2018). International 
    Economics: Theory and Policy. Pearson.

LABORATORIO 1 - PUNTO 2: SIMULACIÓN DE CRÉDITO DE MAQUINARIA (USD)
PROYECTO: Derivados Financieros
FECHA: 26 de marzo de 2026
================================================================================

================================================================================
1. INFORMACIÓN GENERAL DEL PROYECTO
================================================================================

1.1 Contexto de la Inversión
----------------------------
Este punto del laboratorio presenta la simulación de un crédito para la 
adquisición de maquinaria amarilla, denominado en dólares estadounidenses (USD), 
con el objetivo de analizar las condiciones de financiamiento en el mercado 
americano y su impacto financiero.

[TABLA 1: Datos Generales del Crédito]

<img width="686" height="266" alt="image" src="https://github.com/user-attachments/assets/95010b0e-fb53-414a-9607-53920e76336d" />

<img width="490" height="394" alt="image" src="https://github.com/user-attachments/assets/f572f149-1378-4350-b17c-f7403b821e44" />

Propósito: Mostrar la comparación de tasas de interés entre diferentes 
           entidades financieras estadounidenses
Justificación: Permite visualizar por qué se seleccionó la tasa del 8.25% 
               como competitiva en el mercado

1.2 Justificación del Financiamiento en USD
-------------------------------------------
Análisis Estudiantil:
La decisión de financiar en dólares en lugar de pesos colombianos se basa en 
varias razones importantes:

✓ Tasas de interés más bajas: El mercado estadounidense ofrece tasas 
  significativamente menores (8.25%) comparado con las tasas en Colombia 
  (BanRep 9.25% + spreads bancarios que pueden llevar la tasa a 12-15%)

✓ Estabilidad monetaria: El USD es una moneda de reserva internacional con 
  menor volatilidad que el COP

✓ Acceso a mejores condiciones: Las entidades estadounidenses ofrecen 
  condiciones competitivas para equipamiento

⚠️ PERO EXISTE UN RIESGO IMPORTANTE:
El riesgo cambiario es crucial. Si el peso colombiano se devalúa (la TRM 
sube), el costo real del crédito en pesos aumentará significativamente. 
Este riesgo se abordará en el Punto 5 del laboratorio con estrategias de 
cobertura.

================================================================================
2. ANÁLISIS DEL MERCADO DE TASAS DE INTERÉS
================================================================================

2.1 Comparativo de Entidades Financieras
----------------------------------------
Se realizó un estudio de mercado de 6 diferentes fuentes de financiamiento 
en Estados Unidos para créditos de maquinaria y equipamiento.

[TABLA 2: Tasas de Interés del Mercado Americano]

<img width="878" height="190" alt="image" src="https://github.com/user-attachments/assets/d9a85d7d-6c2a-4376-82c4-2d8d2f73ca69" />

<img width="481" height="388" alt="image" src="https://github.com/user-attachments/assets/7585cce9-8d48-4c71-8321-f8b184a059c3" />

Propósito: Visualizar los rangos mínimo y máximo de tasas por entidad
Justificación: Muestra la dispersión de tasas y la posición de la tasa 
               seleccionada (8.25%) dentro del mercado

2.2 Análisis de la Tasa Seleccionada
------------------------------------
Tasa Seleccionada: 8.25% anual
Fuente: Equipment Loan - BankRate.com

Análisis Comparativo (Perspectiva Estudiantil):

✓ VENTAJAS DE LA SELECCIÓN:
  • Tasa por debajo del promedio general del mercado (8.25% vs 8.75%)
  • Menor que SBA 7(a) Loan (10.50%) - nos ahorra 2.25%
  • Menor que Commercial Bank Loan (8.75%) - nos ahorra 0.50%
  • Competitiva frente a grandes bancos como Wells Fargo (8.12%) y 
    Bank of America (8.37%)

⚠️ CONSIDERACIONES:
  • No es la tasa más baja del mercado (JPMorgan ofrece 7.87%)
  • Sin embargo, JPMorgan tiene un rango más estrecho (7.00% - 8.75%), 
    lo que podría indicar requisitos más estrictos de aprobación
  • La diferencia de 0.38% con JPMorgan representa aproximadamente 
    $1,560 USD adicionales en 10 años, lo cual no es significativo 
    comparado con posibles mayores exigencias

Referencias:
- BankRate.com - Commercial Equipment Loans (Marzo 2026)
- wellsfargo.com, jpmorganchase.com, bankofamerica.com (Marzo 2026)
- SBA.gov - Small Business Administration (2026)

================================================================================
3. ESTRUCTURA Y CÁLCULO DEL CRÉDITO
================================================================================

3.1 Cálculo de la Cuota Trimestral
----------------------------------
Sistema de Amortización: Francés (Cuota Constante)

Explicación para estudiantes:
En el sistema francés, pagamos la MISMA CUOTA cada trimestre durante los 
10 años. Al inicio, la mayor parte de esa cuota va a pagar INTERESES y 
poco va a reducir el capital. Con el tiempo, esto se va invirtiendo: cada 
vez pagamos menos intereses y más capital. Es como una montaña rusa que 
comienza con muchos intereses y termina con mucha amortización.

Fórmula Aplicada:
A = P × [i(1+i)^n] / [(1+i)^n - 1]

Donde:
• A = Cuota periódica constante = $3,169.62 USD
• P = Principal del crédito = $85,763.60 USD
• i = Tasa periódica trimestral = 8.25% ÷ 4 = 2.0625%
• n = Número de períodos = 10 años × 4 = 40 trimestres

Cálculo Paso a Paso:
Paso 1: i = 8.25% / 4 = 0.020625 (tasa trimestral)
Paso 2: (1+i)^n = (1.020625)^40 = 2.260977
Paso 3: i(1+i)^n = 0.020625 × 2.260977 = 0.046633
Paso 4: (1+i)^n - 1 = 2.260977 - 1 = 1.260977
Paso 5: [i(1+i)^n] / [(1+i)^n - 1] = 0.046633 / 1.260977 = 0.036957
Paso 6: A = 85,763.60 × 0.036957 = $3,169.62 USD por trimestre

Equivalente en pesos:
Cuota Trimestral (COP) = $3,169.62 × $3,688.46 = $11,691,012.50 COP

<img width="501" height="297" alt="image" src="https://github.com/user-attachments/assets/b2fd8b85-21a3-4be5-8a41-c6ed47a869f7" />

Propósito: Mostrar cómo evoluciona la proporción entre interés y 
           amortización en cada cuota
Justificación: Ilustra visualmente el comportamiento del sistema francés: 
               mayor interés al inicio (área roja), mayor amortización 
               al final (área azul)
               
3.2 Resumen Financiero del Crédito
----------------------------------
[TABLA 3: Resumen del Crédito]

<img width="840" height="163" alt="image" src="https://github.com/user-attachments/assets/272b3b3b-c86b-42cd-8a04-092578e787e6" />

Análisis del Costo Financiero (Explicación Estudiantil):
El 47.83% significa que por cada $100 USD que pedimos prestados, terminamos 
pagando $47.83 USD adicionales en intereses a lo largo de los 10 años.

¿Es mucho o poco?
• Comparado con un crédito de consumo en Colombia (que puede tener tasas 
  del 18-25% anual), es MUY ECONÓMICO
• Comparado con un crédito de vivienda (que puede tener tasas del 8-12% 
  anual en Colombia), es COMPETITIVO
• En 10 años, pagar casi la mitad del valor prestado en intereses es 
  RAZONABLE para un crédito de maquinaria

================================================================================
4. TABLA DE AMORTIZACIÓN DETALLADA
================================================================================

4.1 Análisis de los Primeros 10 Períodos
----------------------------------------
[TABLA 4: Primeros 10 Trimestres de Amortización]

<img width="902" height="277" alt="image" src="https://github.com/user-attachments/assets/b31360d8-6542-440c-ac69-d46e89e16d7e" />

Análisis del Comportamiento Inicial (Perspectiva Estudiantil):

📊 PROPORCIÓN INTERÉS vs AMORTIZACIÓN:
• Período 1: 55.8% intereses ($1,768.87) / 44.2% amortización ($1,400.74)
  → Al inicio, más de la mitad de nuestra cuota se va SOLO en intereses
• Período 10: 46.9% intereses ($1,486.35) / 53.1% amortización ($1,683.27)
  → Después de 2.5 años, ya pagamos más capital que intereses

💡 LECCIÓN IMPORTANTE:
Los primeros años del crédito son los MÁS CAROS porque pagamos muchos 
intereses y reducimos poco el capital. Es como correr en arenas movedizas: 
avanzamos poco al principio.

<img width="497" height="296" alt="image" src="https://github.com/user-attachments/assets/c021e791-eee3-4f0e-878d-c94e8febc5e7" />

Propósito: Mostrar la disminución del saldo pendiente a lo largo del tiempo
Justificación: Visualiza cómo el capital se va amortizando progresivamente 
               hasta llegar a cero en el trimestre 40

4.2 Análisis de los Últimos 10 Períodos
---------------------------------------
[TABLA 5: Últimos 10 Trimestres de Amortización]

<img width="904" height="278" alt="image" src="https://github.com/user-attachments/assets/b451ee0c-b9de-427c-ad3f-093f4f4190f1" />

Análisis del Comportamiento Final (Explicación Estudiantil):

📊 PROPORCIÓN INTERÉS vs AMORTIZACIÓN:
• Período 31: 18.5% intereses ($585.31) / 81.5% amortización ($2,584.31)
  → En el año 8, la MAYORÍA de la cuota va a reducir capital
• Período 40: 2.0% intereses ($64.05) / 98.0% amortización ($3,105.57)
  → ¡En la última cuota casi TODO va a capital!

🎯 COMPARACIÓN IMPACTANTE:
• Intereses período 1: $1,768.87
• Intereses período 40: $64.05
• Reducción: 96.4% menos intereses al final

CONCLUSIÓN: El sistema francés beneficia al banco al inicio (cobran muchos 
intereses) y nos beneficia a nosotros al final (pagamos poco interés y 
amortizamos rápido).

<img width="512" height="315" alt="image" src="https://github.com/user-attachments/assets/b953e59d-65f6-4ea3-a736-341a681088a6" />

Propósito: Mostrar la evolución acumulada de intereses pagados vs capital 
           amortizado
Justificación: Permite visualizar el punto de cruce donde la amortización 
               acumulada supera a los intereses acumulados (aproximadamente 
               en el período 15)

4.3 Análisis Anual del Crédito
------------------------------
[TABLA 6: Distribución Anual de Intereses y Amortización]

<img width="868" height="274" alt="image" src="https://github.com/user-attachments/assets/281dda42-1aae-4754-b49d-e6b50e082ac7" />

<img width="500" height="302" alt="image" src="https://github.com/user-attachments/assets/c5a6032b-0b52-4a75-ae80-c0617067e633" />

Propósito: Comparar visualmente la proporción de interés vs amortización 
           por año
Justificación: Muestra claramente la transición del sistema francés: 
               barras rojas (interés) decrecientes, barras azules 
               (amortización) crecientes

Análisis de la Evolución Anual (Perspectiva Estudiantil):

📈 PUNTOS CLAVE POR ETAPAS:

AÑOS 1-2 (Etapa Cara):
• Más del 50% de los pagos van a intereses
• Menor reducción del capital
• Sensación de "no avanzar"

AÑO 5 (Punto de Inflexión):
• 63.2% va a amortización
• Por primera vez, pagamos MÁS capital que intereses
• El crédito comienza a "valer la pena"

AÑOS 9-10 (Etapa Final):
• Solo 4.9-12.4% son intereses
• 87.6-95.1% amortiza capital
• Liquidamos el crédito rápidamente

💡 LECCIÓN PARA ESTUDIANTES:
Los primeros años de un crédito a largo plazo son los MÁS COSTOSOS. 
Si podemos hacer abonos extras al capital en los primeros 3-5 años, 
ahorramos MUCHOS intereses.

================================================================================
5. INDICADORES DE RENTABILIDAD Y COSTO
================================================================================

5.1 Costo Financiero Total
--------------------------
Cálculo del Costo Financiero:

Costo Financiero = (Total Intereses / Monto Financiado) × 100
Costo Financiero = ($41,021.15 / $85,763.60) × 100
Costo Financiero = 47.83%

Interpretación Estudiantil:
Por cada $100 USD financiados, pagamos $47.83 USD en intereses a lo largo 
de 10 años. Esto equivale a:
• $4.78 USD anuales por cada $100 USD prestados
• $0.40 USD mensuales por cada $100 USD prestados

5.2 Tasa Efectiva Anual (TEA)
-----------------------------
Dado que los pagos son trimestrales con una tasa nominal anual del 8.25%:

Cálculo:
Tasa Trimestral = 8.25% ÷ 4 = 2.0625%

TEA = (1 + i_trimestral)^4 - 1
TEA = (1.020625)^4 - 1
TEA = 1.0851 - 1
TEA = 8.51%

La tasa efectiva anual es del 8.51%, ligeramente superior a la tasa nominal 
del 8.25% debido a la capitalización trimestral.

Explicación para estudiantes:
La TEA es la tasa REAL que pagamos, teniendo en cuenta que los intereses 
se calculan cada trimestre. Es como el "precio verdadero" del dinero.

5.3 Comparación con Alternativas
--------------------------------
[TABLA 7: Comparación con Otras Opciones de Financiamiento]

<img width="872" height="217" alt="image" src="https://github.com/user-attachments/assets/d74b1113-4abb-48fe-8c94-effc0b600048" />

Análisis :

✓ NUESTRA ELECCIÓN FUE INTELIGENTE:
• Comparado con SBA 7(a) Loan: Ahorramos $12,213.52 USD 
  (casi $45 millones de pesos)
• Comparado con Commercial Bank Loan: Ahorramos $2,657.77 USD
  (casi $10 millones de pesos)

⚠️ PERO PODRÍAMOS HABER MEJORADO:
• JPMorgan Chase ofrecería un ahorro adicional de $2,008.70 USD
  (aproximadamente $7.4 millones de pesos)
• Sin embargo, la diferencia de $2,008.70 en 10 años representa 
  solo $200.87 USD por año ($743,000 COP/año)
• Esta diferencia puede no justificar posibles requisitos más estrictos 
  o menores flexibilidades de JPMorgan

================================================================================
6. ANÁLISIS DE SENSIBILIDAD CAMBIARIA
================================================================================

6.1 Impacto de la Variación de la TRM
-------------------------------------
Dado que el crédito está denominado en USD pero la empresa probablemente 
genera ingresos en COP, es fundamental analizar el impacto de diferentes 
escenarios de la tasa de cambio.

[TABLA 8: Escenarios de Impacto Cambiario]

<img width="914" height="229" alt="image" src="https://github.com/user-attachments/assets/5980edc6-f7df-420e-866f-b0322b3c8486" />

Análisis de Riesgo :

⚠️ RIESGO ALCISTA (TRM aumenta - PESO SE DEVALÚA):
• Si la TRM sube a $4,200 (escenario alcista del Punto 1 con 25% 
  probabilidad), el costo en pesos aumenta $64.9 millones
• Esto representa un impacto SIGNIFICATIVO del 13.9% sobre el valor base
• Equivale a casi $6.5 millones de pesos adicionales por año
• ¡Es como pagar un carro adicional al final del crédito!

✅ RIESGO BAJISTA (TRM disminuye - PESO SE APRECIA):
• Si la TRM baja a $3,500, nos ahorramos $23.9 millones
• Beneficio del 5.1% sobre el valor base
• Pero NO debemos contar con esto porque es menos probable (15%)

🎯 CONCLUSIÓN CRÍTICA:
El riesgo de que la TRM suba es REAL y SIGNIFICATIVO. Según el Punto 1:
• Escenario Base (55%): TRM $3,900 → Pagamos $26.8M más
• Escenario Alcista (25%): TRM $4,325 → Pagamos $64.9M más
• Escenario Crisis (5%): TRM $4,900 → Pagamos $120M más

Por esto es OBLIGATORIO cubrir el riesgo cambiario con forwards o opciones, 
tema que veremos en el Punto 5 del laboratorio.

================================================================================
7. CONCLUSIONES DEL PUNTO 2
================================================================================

7.1 Hallazgos Principales
-------------------------
✓ 1. TASA COMPETITIVA:
  • La tasa del 8.25% anual es competitiva dentro del mercado estadounidense
  • Se ubica en el percentil 40 (mejor que 60% de las opciones analizadas)
  • Genera un ahorro significativo vs opciones como SBA 7(a) Loan ($12,213 USD)

✓ 2. ESTRUCTURA DE AMORTIZACIÓN CLARA:
  • El sistema francés proporciona predictibilidad con cuotas constantes
  • La transición de interés a amortización es progresiva y predecible
  • Al año 5, más del 63% de la cuota amortiza capital

✓ 3. COSTO FINANCIERO RAZONABLE:
  • El 47.83% de costo total en 10 años es aceptable para financiamiento 
    de largo plazo
  • Equivale a un costo anual del 4.78% sobre el saldo promedio
  • Menor que alternativas locales en Colombia

✓ 4. BENEFICIOS FISCALES:
  • Los intereses son deducibles de impuestos (dependiendo de la jurisdicción)
  • Esto reduce el costo efectivo real del crédito
  • Puede representar un ahorro adicional del 20-35% en impuestos

7.2 Recomendaciones
-------------------
📌 1. CUBRIR RIESGO CAMBIARIO (CRÍTICO):
   • Implementar estrategia de cobertura con forwards o opciones cambiarias
   • El riesgo de que la TRM suba a $4,200+ es real (25% probabilidad)
   • Se recomienda cubrir al menos 50-75% del flujo de pagos
   • Sin cobertura, el proyecto es MUY RIESGOSO

📌 2. PREPAGO PARCIAL:
   • Considerar prepago de capital en años 3-5 cuando la proporción de 
     amortización aumenta
   • Un abono extra de $10,000 USD en el año 3 podría ahorrar 
     aproximadamente $5,000-7,000 USD en intereses
   • Esto reduciría significativamente el total de intereses pagados

📌 3. MONITOREO DE TASAS:
   • Evaluar anualmente la posibilidad de refinanciación si las tasas de 
     mercado bajan
   • Un descenso de 0.5% en la tasa generaría ahorros de aproximadamente 
     $2,500 USD
   • Mantenerse informado sobre condiciones del mercado

📌 4. FLUJO DE CAJA:
   • Asegurar que los ingresos en COP sean suficientes para cubrir las 
     cuotas en USD
   • Mantener un colchón de liquidez del 10-15% sobre el valor de las cuotas
   • Tener un fondo de emergencia de al menos 6 cuotas ($19,000 USD)

7.3 Reflexión Final 
---------------------------------------------
El crédito en USD representa una OPORTUNIDAD DE FINANCIAMIENTO A TASA 
COMPETITIVA, pero conlleva un RIESGO CAMBIARIO IMPORTANTE que no puede 
ignorarse.

VENTAJA PRINCIPAL:
• Tasa del 8.25% vs tasas locales superiores al 12-15% en Colombia
• Ahorro potencial de $12,000-15,000 USD en 10 años

DESVENTAJA PRINCIPAL:
• Exposición al riesgo de devaluación del peso colombiano
• Posible incremento de $26-65 millones de pesos si la TRM sube

CONCLUSIÓN FINAL:
El financiamiento es RECOMENDABLE SIEMPRE Y CUANDO se implementen mecanismos 
de cobertura cambiaria adecuados (forwards), tema que desarrollaremos en el 
Punto 5 del laboratorio. Sin cobertura, el riesgo es demasiado alto y podría 
convertir un buen negocio en una pérdida significativa.

================================================================================
REFERENCIAS BIBLIOGRÁFICAS
================================================================================

1. BankRate.com. (2026). Commercial Equipment Loans Interest Rates. 
   www.bankrate.com
2. Small Business Administration (SBA). (2026). SBA 7(a) Loan Program Rates. 
   www.sba.gov
3. Wells Fargo. (2026). Equipment Financing Rates and Terms. 
   www.wellsfargo.com
4. JPMorgan Chase. (2026). Business Loan Interest Rates. 
   www.jpmorganchase.com
5. Bank of America. (2026). Equipment Loan Products. 
   www.bankofamerica.com
6. Brealey, R. A., Myers, S. C., & Allen, F. (2020). Principles of Corporate 
   Finance. McGraw-Hill Education.

7. Gitman, L. J., & Zutter, C. J. (2019). Principles of Managerial Finance. 
   Pearson.
8. Banco de la República de Colombia. (2026). Tasa Representativa del 
   Mercado (TRM). www.banrep.gov.co
9. Federal Reserve. (2026). Commercial & Industrial Loan Rates. 
   www.federalreserve.gov
10. Investopedia. (2026). French Amortization System. 
    www.investopedia.com

LABORATORIO 1 - PUNTO 3: CRÉDITO EN PESOS Y ANÁLISIS DE COMPORTAMIENTO
PROYECTO: Derivados Financieros
FECHA: 26 de marzo de 2026
================================================================================

================================================================================
1. INFORMACIÓN GENERAL DEL ANÁLISIS
================================================================================

1.1 Contexto del Estudio
------------------------
Este punto del laboratorio analiza el comportamiento del crédito en dólares 
cuando se convierte a pesos colombianos, considerando la variación de la TRM 
a lo largo de los 10 años del crédito. El objetivo es cuantificar el impacto 
del riesgo cambiario en el costo real del financiamiento.

[TABLA 1: Datos Generales del Análisis]

<img width="699" height="186" alt="image" src="https://github.com/user-attachments/assets/1b15a233-5762-4c81-add8-f4173b8c71b6" />

<img width="3570" height="1489" alt="punto3_trm_proyectada" src="https://github.com/user-attachments/assets/2b28e1ae-8aa0-4fc7-b192-6538bec519ff" />

Propósito: Mostrar la evolución proyectada de la TRM durante los 40 trimestres
Justificación: Permite visualizar la tendencia alcista de la TRM y su 
               volatilidad a lo largo del período del crédito

1.2 Metodología de Proyección de la TRM
---------------------------------------
Análisis Estudiantil:
Para proyectar la TRM utilizamos un modelo que combina:

✓ Tendencia histórica: Basada en el comportamiento de los últimos 5 años
✓ Factores fundamentales: Diferencial de tasas, riesgo país, petróleo
✓ Escenarios probabilísticos: Base (55%), Alcista (25%), Bajista (15%)
✓ Componente estocástico: Volatilidad del 14.07% anual

La proyección muestra una TRM que inicia en $3,688.46 y alcanza un máximo 
proyectado de $5,133.03 (período 13), con una tendencia general alcista 
pero con fluctuaciones significativas.

================================================================================
2. COMPARATIVO CRÉDITO USD vs COP
================================================================================

2.1 Análisis de Cuotas en Diferentes Monedas
--------------------------------------------
[TABLA 2: Comparativo Total a Pagar]

<img width="830" height="147" alt="image" src="https://github.com/user-attachments/assets/6836559a-b26d-4e66-8335-97066fcc3271" />


<img width="3570" height="1488" alt="punto3_comparativo_usd_cop" src="https://github.com/user-attachments/assets/26c6307a-9828-4136-961a-c994cd71a480" />


Propósito: Comparar visualmente la cuota constante en USD vs la cuota 
           variable en COP (dividida entre 1000 para mejor visualización)
Justificación: Muestra claramente cómo la cuota en pesos varía significativamente 
               debido a los cambios en la TRM, mientras que en dólares se mantiene constante
2.2 Análisis de la Variación de Cuotas en Pesos
-----------------------------------------------
Explicación para Estudiantes:

📊 OBSERVACIONES CLAVE:

1. CUOTA EN USD (Línea Azul):
   • Se mantiene CONSTANTE en $3,169.62 durante los 40 trimestres
   • Esto es el sistema francés: siempre pagas lo mismo en dólares
   • Fácil de planificar en moneda extranjera

2. CUOTA EN COP (Línea Roja - escalada/1000):
   • Inicia en ~$12,090 (período 1)
   • Alcanza máximo de ~$16,330 (período 13)
   • Termina en ~$14,282 (período 40)
   • Variación total: +18.1% entre el primer y último período

💡 ¿POR QUÉ CAMBIA LA CUOTA EN PESOS?
Porque aunque pagas siempre los mismos dólares ($3,169.62), la TRM cambia:
• Si la TRM SUBE → Pagas MÁS pesos por los mismos dólares
• Si la TRM BAJA → Pagas MENOS pesos por los mismos dólares

Ejemplo práctico:
• Período 1: $3,169.62 × $3,814.57 = $12,090,728 COP
• Período 13: $3,169.62 × $5,133.03 = $16,269,747 COP
  → ¡$4.18 millones de pesos MÁS por la misma cuota en dólares!

================================================================================
3. IMPACTO DEL RIESGO CAMBIARIO
================================================================================

3.1 Cálculo del Impacto Cambiario
---------------------------------
[TABLA 3: Impacto de la Variación de la TRM]

<img width="859" height="140" alt="image" src="https://github.com/user-attachments/assets/991b32af-df40-4c46-b316-d2295280b726" />

Análisis Estudiantil:
Este es el resultado MÁS IMPORTANTE del Punto 3. Nos muestra que:

💰 EL RIESGO CAMBIARIO NOS CUESTA $111.4 MILLONES DE PESOS

Traducción práctica:
• Si la TRM se hubiera mantenido constante en $3,688.46, habríamos pagado 
  $467.6 millones de pesos en total
• Pero como la TRM subió a un promedio de $4,567.02, terminamos pagando 
  $579.0 millones de pesos
• LA DIFERENCIA DE $111.4 MILLONES es el costo del riesgo cambiario

¿Es mucho o poco?
• Representa un 23.82% adicional sobre el costo base
• Equivale a casi el valor de la maquinaria inicial ($351.5 millones)
• Es como si tuviéramos que comprar UN 23% MÁS de maquinaria

3.2 Desglose del Impacto por Componentes
----------------------------------------
[TABLA 4: Desglose del Impacto Cambiario]

<img width="830" height="136" alt="image" src="https://github.com/user-attachments/assets/d1a13457-c529-402b-8284-46dfcbac5618" />

Explicación para Estudiantes:
El impacto de $111.4 millones se divide en:

1. IMPACTO EN INTERESES ($67.3 millones):
   • Los intereses en dólares son fijos ($41,021.15)
   • Pero al convertirlos a pesos con TRM más alta, cuestan más
   • Representa el 60.5% del impacto total

2. IMPACTO EN CAPITAL ($44.0 millones):
   • El capital en dólares es fijo ($85,763.60)
   • Pero al convertirlo a pesos con TRM más alta, cuesta más
   • Representa el 39.5% del impacto total
================================================================================
4. ANÁLISIS DE ESCENARIOS DE TRM
================================================================================

4.1 Escenarios Proyectados a 10 Años
------------------------------------
[TABLA 5: Escenarios de TRM - Resumen]

<img width="815" height="123" alt="image" src="https://github.com/user-attachments/assets/06b653f1-3751-4ec8-97aa-e0a70ccdfc4c" />

<img width="3570" height="1488" alt="punto3_escenarios_trm" src="https://github.com/user-attachments/assets/5cdc427f-f6e1-43a6-ae2d-1879581a23de" />

Propósito: Visualizar los tres escenarios (Base, Alcista, Bajista) de la TRM
           junto con la TRM inicial de referencia
Justificación: Permite comparar cómo diferentes escenarios afectan la evolución 
               de la TRM y entender la incertidumbre del mercado cambiario

4.2 Análisis Detallado por Escenario
------------------------------------
Análisis :

📈 ESCENARIO BASE (55% de probabilidad):
• TRM promedio: $4,567.02
• Variación: +22.16%
• Costo total en COP: $579.0 millones
• Interpretación: Es el escenario MÁS PROBABLE. La TRM sube moderadamente 
  pero con volatilidad. Es realista y alcanzable.

📈 ESCENARIO ALCISTA (25% de probabilidad):
• TRM máxima proyectada: $11,000.00
• Variación: +96.56%
• Costo total en COP: ~$1,240 millones
• Interpretación: Escenario PESIMISTA. Ocurre si hay crisis económica, 
  devaluación fuerte, o problemas fiscales graves en Colombia. 
  ¡Casi se triplica el costo!

📉 ESCENARIO BAJISTA (15% de probabilidad):
• TRM mínima proyectada: $2,000.00
• Variación: -15.95%
• Costo total en COP: ~$393 millones
• Interpretación: Escenario OPTIMISTA. Ocurre si el peso colombiano se 
  fortalece significativamente. Sería lo MEJOR que nos podría pasar.

4.3 Impacto Financiero por Escenario
------------------------------------
[TABLA 6: Impacto Financiero por Escenario]

<img width="812" height="149" alt="image" src="https://github.com/user-attachments/assets/74f0e954-ff69-48fd-a426-4bf421275751" />

Esta tabla muestra la IMPORTANCIA CRÍTICA de cubrir el riesgo cambiario:

✓ En el mejor caso (Bajista): Nos ahorraríamos $186 millones
✓ En el peor caso (Alcista): Perderíamos $661 millones adicionales

La diferencia entre el mejor y peor escenario es de $847 MILLONES DE PESOS.
¡Esto justifica completamente el uso de forwards para protegernos!

================================================================================
5. TABLA DE AMORTIZACIÓN EN PESOS (PRIMEROS 10 PERÍODOS)
================================================================================

5.1 Detalle de los Primeros 10 Trimestres
-----------------------------------------
[TABLA 7: Amortización - Primeros 10 Períodos (COP)]

<img width="918" height="278" alt="image" src="https://github.com/user-attachments/assets/773e3ed7-09c9-4bd9-a927-3a761416657f" />

📊 OBSERVACIONES IMPORTANTES:

1. CRECIMIENTO DE LAS CUOTAS:
   • Período 1: $12.09 millones
   • Período 10: $16.05 millones
   • Aumento: +32.7% en solo 2.5 años

2. COMPONENTE DE INTERESES:
   • Los intereses en pesos son ALTOS ($6.7-7.6 millones por trimestre)
   • Esto se debe a la TRM proyectada en aumento
   • Representan 55-60% de la cuota total

3. SALDO PENDIENTE:
   • Aunque pagamos, el saldo en pesos AUMENTA en algunos períodos
   • Período 3→4: De $326M a $344.9M (+$18.9M)
   • Esto pasa cuando la TRM sube más rápido de lo que amortizamos

💡 LECCIÓN CLAVE:
En un crédito en dólares con TRM volátil, puedes estar pagando puntualmente 
pero deber MÁS en pesos si la TRM sube significativamente. Esto se llama 
"amortización negativa" y es un riesgo real del financiamiento en moneda 
extranjera.

================================================================================
6. ANÁLISIS DE SENSIBILIDAD Y VOLATILIDAD
================================================================================

6.1 Métricas de Volatilidad de la TRM
-------------------------------------
[TABLA 8: Estadísticas de la TRM Proyectada]

<img width="778" height="188" alt="image" src="https://github.com/user-attachments/assets/815767da-55dd-4cf6-a762-5d6f3ab0d72b" />

📊 ¿QUÉ NOS DICEN ESTAS CIFRAS?

1. RANGO DE VARIACIÓN ($1,444.57):
   • La diferencia entre el mínimo y máximo es SIGNIFICATIVA
   • Representa un 39.16% de variación
   • Es como si el precio de un carro de $100 millones pudiera subir a 
     $139 millones o bajar a $61 millones

2. VOLATILIDAD ANUAL (14.07%):
   • Indica qué tanto "se mueve" la TRM en un año
   • 14% es ALTA volatilidad para una variable financiera
   • Compara: La inflación usualmente varía 1-3% anual
   • La TRM puede variar 14% → ¡Mucho más riesgosa!

3. DESVIACIÓN ESTÁNDAR ($412.35):
   • En promedio, la TRM se desvía $412 de su valor esperado
   • Esto significa que en cualquier trimestre, la TRM podría ser 
     $412 más alta o más baja de lo proyectado
   • Impacto en una cuota: $3,169.62 × $412 = $1.3 millones adicionales

6.2 Períodos Críticos de Mayor Volatilidad
------------------------------------------
Análisis :

⚠️ PERÍODOS DE ALTO RIESGO IDENTIFICADOS:

1. PERÍODOS 10-14 (Año 3-4):
   • TRM alcanza $5,133.03 (máximo histórico proyectado)
   • Variación: +39.16% vs TRM inicial
   • Cuota en pesos: ~$16.3 millones
   • Recomendación: Tener liquidez adicional en este período

2. PERÍODOS 32-36 (Año 8-9):
   • Segundo pico de volatilidad
   • TRM proyectada: $4,800-5,000
   • Cuota en pesos: ~$15.5 millones
   • Recomendación: Revisar cobertura en este período

3. PERÍODOS 1-4 (Año 1):
   • Aumento rápido inicial
   • TRM sube de $3,688 a $4,311
   • Cuota sube de $12.1M a $13.7M (+13.2%)
   • Recomendación: Impacto inicial fuerte, planificar flujo de caja

================================================================================
7. COMPARACIÓN CON CRÉDITO LOCAL EN PESOS
================================================================================

7.1 Análisis Comparativo
------------------------
[TABLA 9: Crédito USD vs Crédito COP Local]

<img width="842" height="189" alt="image" src="https://github.com/user-attachments/assets/e13db242-e8a4-4d06-b6ed-d98dffee3877" />

🤔 ¿CONVIENE MÁS EL CRÉDITO EN USD O EN COP?

VENTAJAS DEL CRÉDITO EN USD:
✓ Tasa de interés más baja (8.25% vs 13.50%)
✓ Ahorro en intereses de $28.9 millones (sin contar riesgo cambiario)
✓ Acceso a mayor monto de financiamiento
✓ Posibilidad de cobertura con forwards

DESVENTAJAS DEL CRÉDITO EN USD:
✗ Riesgo cambiario de $111.4 millones
✗ Incertidumbre en el flujo de caja
✗ Necesidad de instrumentos de cobertura
✗ Complejidad administrativa

CONCLUSIÓN DEL ANÁLISIS:
• Sin cobertura: Crédito en COP es MÁS SEGURO (aunque más costoso en tasa)
• Con cobertura (forwards): Crédito en USD puede ser MÁS ECONÓMICO
• Diferencia final: $47.7 millones a favor del crédito en COP sin cubrir
• PERO con forwards adecuados, el crédito en USD podría ahorrar dinero

💡 RECOMENDACIÓN:
El crédito en USD SOLO es recomendable si:
1. Se cubre el riesgo cambiario con forwards
2. La empresa genera ingresos en dólares
3. Se tiene capacidad de absorber volatilidad

================================================================================
8. CONCLUSIONES DEL PUNTO 3
================================================================================

8.1 Hallazgos Principales
-------------------------
✓ 1. IMPACTO CAMBIARIO SIGNIFICATIVO:
   • El riesgo cambiario representa $111.4 millones adicionales (23.82%)
   • Esto supera el ahorro por la menor tasa de interés
   • Sin cobertura, el crédito en USD es MÁS COSTOSO que uno local

✓ 2. VOLATILIDAD ALTA PROYECTADA:
   • Volatilidad anual del 14.07%
   • Rango de variación de $1,444.57 COP/USD
   • Períodos críticos identificados en años 3-4 y 8-9

✓ 3. INCERTIDUMBRE EN EL FLUJO DE CAJA:
   • Las cuotas en pesos varían entre $12.1M y $16.3M
   • Diferencia de $4.2 millones por trimestre
   • Dificulta la planificación financiera

✓ 4. ESCENARIOS EXTREMOS PREOCUPANTES:
   • Escenario Alcista: Costo de $1,240 millones (+114%)
   • Escenario Bajista: Costo de $393 millones (-32%)
   • Diferencia entre escenarios: $847 millones

✓ 5. AMORTIZACIÓN NEGATIVA POSIBLE:
   • El saldo en pesos puede AUMENTAR aunque se pague
   • Ocurre cuando la TRM sube más rápido que la amortización
   • Riesgo real en períodos de devaluación fuerte

8.2 Recomendaciones
-------------------
📌 1. COBERTURA OBLIGATORIA CON FORWARDS:
   • Contratar forwards para cubrir al menos 75-100% del flujo
   • Esto elimina la incertidumbre y fija el costo en pesos
   • Costo del forward: Usualmente 0.5-2% anual
   • Beneficio: Elimina riesgo de $111.4 millones

📌 2. MONITOREO CONTINUO DE LA TRM:
   • Revisar trimestralmente la evolución vs proyección
   • Ajustar estrategia de cobertura si es necesario
   • Mantener alerta en períodos críticos (10-14, 32-36)

📌 3. FONDO DE CONTINGENCIA:
   • Mantener reservas por $50-70 millones
   • Para absorber picos de volatilidad no cubiertos
   • Representa 3-5 cuotas adicionales

📌 4. EVALUAR INGRESOS EN DÓLARES:
   • Si la empresa exporta o recibe USD, el riesgo se reduce
   • Los ingresos en dólares cubren naturalmente el crédito
   • Esto se llama "cobertura natural"

📌 5. CONSIDERAR CRÉDITO HÍBRIDO:
   • 50% en USD con forwards + 50% en COP
   • Diversifica el riesgo
   • Balance entre tasa baja y estabilidad

8.3 Reflexión Final (Perspectiva Estudiantil)
---------------------------------------------
El Punto 3 nos enseña una lección FUNDAMENTAL en finanzas internacionales:

"UNA TASA DE INTERÉS BAJA NO SIEMPRE SIGNIFICA UN CRÉDITO MÁS BARATO"

En este caso:
• Crédito USD: Tasa 8.25% → PERO con riesgo de $111.4 millones
• Crédito COP: Tasa 13.50% → PERO sin riesgo cambiario

La decisión correcta depende de:
1. Nuestra capacidad de gestionar el riesgo
2. Nuestra disposición a pagar por certidumbre
3. Nuestras expectativas sobre la TRM
4. Nuestra habilidad para usar instrumentos de cobertura

CONCLUSIÓN FINAL:
El crédito en dólares es una HERRAMIENTA PODEROSA pero PELIGROSA si no se 
gestiona adecuadamente. Sin forwards, es una APUESTA sobre la TRM. Con 
forwards, es una herramienta de financiamiento inteligente.

================================================================================
REFERENCIAS BIBLIOGRÁFICAS
================================================================================

1. Banco de la República de Colombia. (2026). Tasa Representativa del 
   Mercado (TRM) Histórica. www.banrep.gov.co
2. Hull, J. C. (2022). Options, Futures, and Other Derivatives. 
   Pearson Education.
3. Eiteman, D. K., Stonehill, A. I., & Moffett, M. H. (2021). 
   Multinational Business Finance. Pearson.
4. Madura, J. (2020). International Financial Management. 
   Cengage Learning.
5. Ministerio de Hacienda de Colombia. (2026). Marco Fiscal de 
   Mediano Plazo.
6. FMI (Fondo Monetario Internacional). (2026). World Economic Outlook 
   y Proyecciones para Colombia. www.imf.org
7. Bloomberg Terminal. (2026). Datos de Mercados Cambiarios y 
   Proyecciones de TRM.
8. Investopedia. (2026). Currency Risk and Exchange Rate Exposure. 
   www.investopedia.com
9. Federal Reserve. (2026). Exchange Rates and Economic Indicators. 
   www.federalreserve.gov
10. Asociación Bancaria de Colombia. (2026). Tasas de Interés de 
    Referencia para Créditos en Pesos.

LABORATORIO 1 - PUNTO 4: SIMULACIÓN MONTE CARLO - MOVIMIENTO BROWNIANO GEOMÉTRICO
PROYECTO: Derivados Financieros
FECHA: 26 de marzo de 2026
================================================================================

================================================================================
1. INFORMACIÓN GENERAL
================================================================================

1.1 Objetivo del Punto 4
------------------------
Realizar una simulación Monte Carlo utilizando Movimiento Browniano Geométrico 
(MBG) para proyectar la TRM a 5 años, comparando dos distribuciones: Normal 
y T-Student, con el fin de evaluar el riesgo cambiario del crédito en USD.

1.2 Período de Análisis
-----------------------
• Datos históricos: 2019-2024 (5 años)
• Período de proyección: 5 años (60 meses)
• Número de simulaciones: 1,000 trayectorias
• Fecha inicial TRM: $3,688.46 COP/USD (26 de marzo de 2026)

<img width="3508" height="1712" alt="grafica_serie_historica" src="https://github.com/user-attachments/assets/444008d6-557c-45e6-8922-186bd264ef12" />

Propósito: Mostrar la evolución histórica de la TRM durante los últimos 5 años
Justificación: Permite visualizar la tendencia, volatilidad y eventos extremos 
               que han afectado la tasa de cambio

================================================================================
2. ANÁLISIS ESTADÍSTICO DE RETORNOS
================================================================================

2.1 Estadísticos Descriptivos de Retornos Mensuales
---------------------------------------------------
[TABLA 1: Estadísticos de Retornos]

<img width="777" height="207" alt="image" src="https://github.com/user-attachments/assets/4219097a-9a4f-48c0-ad98-375ab0f6fac5" />

📊 INTERPRETACIÓN DE LOS ESTADÍSTICOS:

✓ MEDIA POSITIVA (1.20% mensual):
  • La TRM ha tenido una tendencia alcista en promedio
  • Esto representa una devaluación promedio del peso colombiano
  • Equivale a aproximadamente 15.4% anual (1.012^12 - 1)

✓ VOLATILIDAD ALTA (6.02% mensual):
  • La desviación estándar es 5 veces mayor que la media
  • Esto indica ALTO RIESGO en la proyección
  • Los retornos son muy dispersos alrededor del promedio

✓ ASIMETRÍA POSITIVA (Skewness = 2.50):
  • La distribución tiene cola larga hacia la derecha
  • Hay más probabilidad de devaluaciones extremas que apreciaciones
  • Los eventos extremos son principalmente alzas de la TRM

✓ CURTOSIS MUY ALTA (Kurtosis = 13.25):
  • MUCHO mayor que 3 (valor de distribución normal)
  • Indica "colas pesadas" (fat tails)
  • Los eventos extremos son MÁS FRECUENTES de lo que predice una normal
  • ESTO JUSTIFICA USAR DISTRIBUCIÓN T-STUDENT

✓ RANGO EXTREMO (-12.70% a +32.30%):
  • El retorno máximo es 2.5 veces mayor en magnitud que el mínimo
  • Confirma la asimetría positiva
  • Las devaluaciones bruscas son más intensas que las apreciaciones

<img width="5970" height="3435" alt="grafica_dashboard" src="https://github.com/user-attachments/assets/d38e2665-78aa-41ed-8a0b-8ca79ed0b530" />

Propósito: Panel de 6 gráficos mostrando:
           1. Serie Histórica TRM
           2. Distribución Retornos
           3. QQ-Plot
           4. BMG - Normal
           5. BMG - T-Student
           6. Distribución Final
Justificación: Proporciona una visión completa del análisis estadístico y 
               las simulaciones realizadas
================================================================================
3. METODOLOGÍA DE SIMULACIÓN
================================================================================

3.1 Movimiento Browniano Geométrico (MBG)
-----------------------------------------
Fórmula del MBG:
S(t) = S(0) × exp[(μ - σ²/2)t + σ×W(t)]

Donde:
• S(t) = TRM en el tiempo t
• S(0) = TRM inicial ($3,688.46)
• μ = Media de retornos (0.0120276344 mensual)
• σ = Desviación estándar (0.0601888064 mensual)
• W(t) = Proceso de Wiener (movimiento browniano estándar)
• t = Tiempo en meses (0 a 60)

3.2 Distribuciones Comparadas
-----------------------------
a) DISTRIBUCIÓN NORMAL:
   • Supone retornos normalmente distribuidos
   • Colas delgadas (subestima eventos extremos)
   • Más simple computacionalmente
   
b) DISTRIBUCIÓN T-STUDENT:
   • Considera colas pesadas (fat tails)
   • Mejor captura eventos extremos
   • Más conservadora en estimación de riesgo
   • Justificada por kurtosis = 13.25

3.3 Parámetros de Simulación
----------------------------
• Número de trayectorias: 1,000
• Período: 60 meses (5 años)
• Paso de tiempo: 1 mes
• Semilla: Aleatoria
• TRM inicial: $3,688.46 COP/USD

================================================================================
4. RESULTADOS DE LA SIMULACIÓN
================================================================================

4.1 Estadísticos de la TRM Final (Mes 60)
-----------------------------------------
[TABLA 2: Comparación Distribuciones - TRM Final]

<img width="834" height="186" alt="image" src="https://github.com/user-attachments/assets/72551c30-5294-4343-91b3-61fc16f65a69" />

📊 COMPARACIÓN DE RESULTADOS:

✓ MEDIA SIMILAR:
  • Normal: $12,333.31
  • T-Student: $11,935.81
  • Diferencia: $397.50 (3.23%)
  • Ambas proyectan una TRM final de ~$12,000
  
✓ VOLATILIDAD DIFERENTE:
  • Normal: $6,482.04 (52.6% de la media)
  • T-Student: $5,731.62 (48.0% de la media)
  • La normal muestra mayor dispersión absoluta
  
✓ VaR 95% (Value at Risk):
  • Normal: $4,758.97
  • T-Student: $5,131.61
  • El T-Student es MÁS CONSERVADOR (mayor riesgo)
  • Hay 95% de probabilidad de que la TRM esté POR ENCIMA de estos valores
  
✓ RANGO EXTREMO:
  • Normal: $2,073.29 a $71,548.74 (rango de $69,475.45)
  • T-Student: $2,548.01 a $38,593.56 (rango de $36,045.55)
  • La normal tiene un máximo CASI EL DOBLE que T-Student
  • Pero T-Student tiene un mínimo más alto

<img width="2605" height="1587" alt="grafica_comparacion" src="https://github.com/user-attachments/assets/e09fc41a-81bb-4c87-9574-b7ae004d2189" />

Propósito: Boxplot comparativo mostrando la distribución de la TRM final 
           para Normal vs T-Student
Justificación: Visualiza claramente las diferencias en dispersión, outliers 
               y tendencia central entre ambas distribuciones

4.2 Interpretación del VaR 95%
------------------------------
Análisis Estudiantil:

¿QUÉ SIGNIFICA EL VaR 95%?

Para la distribución T-Student (la más conservadora):
• VaR 95% = $5,131.61
• Interpretación: "Hay 95% de confianza de que la TRM al final del año 5 
  será MAYOR a $5,131.61"
• O alternativamente: "Solo hay 5% de probabilidad de que la TRM esté 
  POR DEBAJO de $5,131.61"

IMPLICACIONES PARA EL CRÉDITO:
• TRM actual: $3,688.46
• VaR 95% (T-Student): $5,131.61
• Variación: +39.13%
• Esto significa que en el PEOR CASO (5% de probabilidad), la TRM podría 
  estar por debajo de $5,131.61, pero en el 95% de los casos estará POR ENCIMA

• Impacto en el crédito:
  - Cuota actual en pesos: $11,691,012 (con TRM $3,688.46)
  - Cuota con VaR 95%: $16,269,747 (con TRM $5,131.61)
  - Aumento: $4,578,735 por trimestre (+39.13%)

================================================================================
5. ANÁLISIS DE TRAYECTORIAS
================================================================================

5.1 Comportamiento de las Simulaciones
--------------------------------------
Análisis Estudiantil:

📈 PATRONES OBSERVADOS:

✓ DIVERGENCIA CRECIENTE:
  • Al inicio (mes 0-12): Las trayectorias están agrupadas
  • Mes 24-36: Comienza la dispersión significativa
  • Mes 48-60: Máxima dispersión (incertidumbre máxima)
  
✓ TENDENCIA GENERAL ALCISTA:
  • La mayoría de trayectorias muestran crecimiento
  • Algunas trayectorias extremas alcanzan valores muy altos
  • Pocas trayectorias muestran depreciación del dólar

✓ OUTLIERS EXTREMOS:
  • Distribución Normal: Algunos valores superan $70,000
  • Distribución T-Student: Máximo alrededor de $38,000
  • La normal es más "extrema" en los valores máximos

5.2 Comparación Visual de Distribuciones
----------------------------------------

<img width="5970" height="3435" alt="grafica_dashboard" src="https://github.com/user-attachments/assets/a832c2d1-b7f4-47a7-bf0d-bdf39971d41c" />

Propósito: Comparar visualmente:
           - Panel 4: Trayectorias BMG con distribución Normal
           - Panel 5: Trayectorias BMG con distribución T-Student
           - Panel 6: Histograma comparativo de distribuciones finales
Justificación: Muestra cómo difieren las trayectorias simuladas y la 
               distribución de resultados finales
================================================================================
6. EVALUACIÓN DE RIESGO CAMBIARIO
================================================================================

6.1 Impacto en el Crédito de Maquinaria
---------------------------------------
[TABLA 3: Impacto del Riesgo Cambiario en el Crédito]

 width="692" height="199" alt="image" src="https://github.com/user-attachments/assets/391d68b6-a51c-4ec5-ab74-03dbfb96bb3d" />

💰 IMPACTO FINANCIERO DRÁMATICO:

✓ ESCENARIO PROMEDIO (T-Student):
  • Cuota actual: $11.69 millones
  • Cuota proyectada (5 años): $37.81 millones
  • Aumento: $26.12 millones (+223.3%)
  
  Esto significa que, EN PROMEDIO, la cuota se TRIPLICARÍA en 5 años.

✓ ESCENARIO VaR 95% (PEOR CASO del 95%):
  • Cuota con VaR 95%: $16.27 millones
  • Aumento: $4.58 millones (+39.13%)
  
  Incluso en un escenario "conservador" (95% confianza), la cuota 
  aumentaría casi 40%.

✓ ESCENARIO MÍNIMO (MEJOR CASO):
  • Cuota mínima T-Student: $8.08 millones
  • Disminución: -$3.61 millones (-30.91%)
  
  En el mejor de los casos, la cuota podría reducirse 31%, pero esto 
  solo ocurre en el 5% de las simulaciones.

6.2 Implicaciones para la Empresa
---------------------------------
Análisis Estudiantil:

⚠️ RIESGOS IDENTIFICADOS:

1. RIESGO DE FLUJO DE CAJA:
   • La cuota trimestral podría aumentar entre $4.58M y $26.12M
   • Esto representa un estrés financiero significativo
   • La empresa debe asegurar ingresos crecientes o cobertura

2. RIESGO DE SOLVENCIA:
   • Si los ingresos no crecen al ritmo de la TRM, la capacidad 
     de pago se deteriora
   • Podría requerirse refinanciación o garantías adicionales

3. RIESGO DE RENTABILIDAD:
   • El costo real del crédito aumenta con la devaluación
   • La rentabilidad del proyecto de maquinaria se reduce
   • Podría volverse inviable si la TRM supera $10,000

4. RIESGO DE COLAS PESADAS:
   • La kurtosis de 13.25 indica que los eventos extremos son 
     MUCHO más probables de lo que sugiere una distribución normal
   • No se puede subestimar el riesgo de devaluaciones bruscas

================================================================================
7. COMPARACIÓN NORMAL vs T-STUDENT
================================================================================

7.1 Ventajas y Desventajas
--------------------------
[TABLA 4: Comparación Metodológica]

<img width="766" height="185" alt="image" src="https://github.com/user-attachments/assets/86e379ba-9205-4ead-a100-4c75fa534a02" />

📊 ¿POR QUÉ T-STUDENT ES MEJOR EN ESTE CASO?

✓ RAZÓN 1: KURTOSIS ALTA (13.25)
  • Una distribución normal tiene kurtosis = 3
  • Nuestros datos tienen kurtosis = 13.25 (4.4 veces mayor)
  • Esto indica COLAS PESADAS que la normal no captura
  
✓ RAZÓN 2: EVENTOS EXTREMOS REALES
  • Histórico muestra saltos bruscos (COVID, crisis políticas)
  • La T-Student permite estos "saltos" con mayor probabilidad
  • La normal los considera "casi imposibles" cuando son frecuentes
  
✓ RAZÓN 3: MÁS CONSERVADOR
  • VaR 95% T-Student ($5,131.61) > VaR 95% Normal ($4,758.97)
  • Es mejor sobreestimar el riesgo que subestimarlo
  • En finanzas, el conservadurismo protege contra sorpresas
  
✓ RAZÓN 4: MÁXIMOS MÁS REALISTAS
  • Normal: $71,548 (poco realista en 5 años)
  • T-Student: $38,593 (alto pero más creíble)
  • La normal genera outliers extremos poco probables

================================================================================
8. CONCLUSIONES DEL PUNTO 4
================================================================================

8.1 Hallazgos Principales
-------------------------
✓ 1. ALTA VOLATILIDAD HISTÓRICA:
   • Desviación estándar de 6.02% mensual
   • Esto genera incertidumbre significativa en proyecciones
   • El riesgo cambiario es REAL y SIGNIFICATIVO

✓ 2. ASIMETRÍA POSITIVA (Skewness = 2.50):
   • Mayor probabilidad de devaluaciones que apreciaciones
   • Las colas derechas son más pesadas
   • El riesgo es principalmente al alza del dólar

✓ 3. COLAS PESADAS (Kurtosis = 13.25):
   • Eventos extremos son 4.4 veces más probables que en una normal
   • JUSTIFICA completamente el uso de T-Student
   • Las crisis generan saltos bruscos en la TRM

✓ 4. PROYECCIÓN ALCISTA:
   • Media T-Student: $11,935.81 en 5 años (+223.5%)
   • Tendencia estructural de devaluación del peso
   • El crédito en USD es RIESGOSO sin cobertura

✓ 5. VaR 95% CONSERVADOR:
   • T-Student: $5,131.61 (+39.13% vs actual)
   • Incluso en escenario "seguro", la TRM sube 39%
   • El riesgo a la baja es limitado

8.2 Recomendaciones
-------------------
📌 1. USAR DISTRIBUCIÓN T-STUDENT:
   • Es estadísticamente más apropiada (kurtosis = 13.25)
   • Proporciona estimaciones más conservadoras
   • Mejor captura el riesgo de colas pesadas

📌 2. COBERTURA CAMBIARIA OBLIGATORIA:
   • La proyección media ($11,935) triplicaría la cuota
   • Se recomienda forwards o opciones para cubrir 75-100%
   • Sin cobertura, el proyecto es MUY RIESGOSO

📌 3. MONITOREO CONTINUO:
   • Recalcular simulaciones trimestralmente
   • Ajustar parámetros (μ, σ) con datos recientes
   • Actualizar estrategia de cobertura según resultados

📌 4. ESTRÉS DE ESCENARIOS:
   • Considerar escenarios de crisis (TRM > $15,000)
   • Evaluar capacidad de pago en escenarios extremos
   • Mantener liquidez adicional como colchón

📌 5. EVALUAR INGRESOS EN USD:
   • Si la empresa exporta, el riesgo se reduce naturalmente
   • Buscar fuentes de ingresos en dólares
   • Cobertura natural es la mejor estrategia

8.3 Reflexión Final
La simulación Monte Carlo con MBG nos ha permitido CUANTIFICAR el riesgo 
cambiario de manera rigurosa. Los resultados son CONTUNDENTES:

✓ Sin cobertura, el crédito en USD es una APUESTA de alto riesgo
✓ La TRM muestra tendencia estructural alcista con alta volatilidad
✓ Los eventos extremos (crisis) son más probables de lo que parece
✓ La distribución T-Student es la herramienta adecuada para este análisis

DECISIÓN RECOMENDADA:
"FINANCIAR EN USD SOLO CON COBERTURA CAMBIARIA ADECUADA (FORWARDS/OPCIONES)"

Sin cobertura, el riesgo de que la cuota se triplique en 5 años es REAL 
y ESTADÍSTICAMENTE SIGNIFICATIVO.

================================================================================
REFERENCIAS BIBLIOGRÁFICAS
================================================================================

1. Hull, J. C. (2022). Options, Futures, and Other Derivatives. Pearson 
   Education.
2. Glasserman, P. (2003). Monte Carlo Methods in Financial Engineering. 
   Springer.
3. McNeil, A. J., Frey, R., & Embrechts, P. (2015). Quantitative Risk 
   Management: Concepts, Techniques and Tools. Princeton University Press.
4. Banco de la República de Colombia. (2026). Tasa Representativa del 
   Mercado (TRM) Histórica. www.banrep.gov.co
5. Shreve, S. E. (2004). Stochastic Calculus for Finance II: 
   Continuous-Time Models. Springer Finance.
6. Jorion, P. (2006). Value at Risk: The New Benchmark for Managing 
   Financial Risk. McGraw-Hill.
7. Investopedia. (2026). Geometric Brownian Motion. www.investopedia.com














